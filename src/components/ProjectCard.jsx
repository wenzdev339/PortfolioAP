...
const ProjectCard = ({ project }) => {
  const { number, title, description, image, tags, youtubeLink } = project;

  const ImageWrapper = ({ children }) =>
    youtubeLink ? (
      <a href={youtubeLink} className="card-image-link" target="_blank" rel="noopener noreferrer">
        {children}
      ...
      <div className="card-image-wrap">{children}</div>
    );
